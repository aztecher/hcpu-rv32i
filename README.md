...
Note: If you need a newer Clash version, simply change the version bounds in `hcpu-rv32i.cabal` and follow the hints given by Stack.

## src/
This is where the source code of the project lives, as specified in `hcpu-rv32i.cabal`. It contains a single file, `Example/Project.hs` which starts with:
...
`hcpu-rv32i.cabal` enabled `NoImplicitPrelude` which enables the use of `Clash.Prelude` here. Next, a function `plus` is defined. It simply adds two numbers. Note that the example (`>>> plus 3 5`) gets executed by the _doctests_ defined for this project and checked for consistency with the result in the documentation (`8`).
...
# Change the license
By default `hcpu-rv32i.cabal` sets its `license` field to `BSD-2-Clause`. You might want to change this.
